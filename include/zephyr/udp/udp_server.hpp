...
#ifndef __ZEPHYR_UDP_SERVER_HPP__
#define __ZEPHYR_UDP_SERVER_HPP__

#if defined(_MSC_VER) && (_MSC_VER >= 1200)
#pragma once
#endif // defined(_MSC_VER) && (_MSC_VER >= 1200)

#include <zephyr/base/server.hpp>
#include <zephyr/udp/udp_session.hpp>
#include <zephyr/base/detail/linear_buffer.hpp>
...
public:
    ...
    /**
		 * @constructor
		 */
    explicit udp_server_impl_t(std::size_t silence_timeout = udp_silence_timeout,
        ...
        , silence_timeout_(udp_silence_timeout)
    {
    }
    ...
    inline bool start(String&& host, StrOrInt&& service)
    {
        return this->derived()._do_start(
            std::forward<String>(host), std::forward<StrOrInt>(service), condition_wrap<void>{});
    }
    ...
    {
        return this->derived()._do_start(
            std::forward<String>(host), std::forward<StrOrInt>(service), condition_wrap<use_kcp_t>(c));
    }
    ...
    {
        return (super::is_started() && this->acceptor_.is_open());
    }
    ...
    {
        return (super::is_stopped() && !this->acceptor_.is_open());
    }
...
public:
    /**
		 ...
		 * Function signature : void(std::shared_ptr<zephyr::udp_session>& session_ptr, std::string_view s)
		 */
     ...
     * Function signature : void(std::shared_ptr<zephyr::udp_session>& session_ptr)
     */
    template<class F, class... C>
    inline derived_t& bind_connect(F&& fun, C&&... obj)
    {
        this->listener_.bind(
            event::connect, observer_t<std::shared_ptr<session_t>&>(std::forward<F>(fun), std::forward<C>(obj)...));
        return (this->derived());
    }
    ...
    {
        this->listener_.bind(
            event::disconnect, observer_t<std::shared_ptr<session_t>&>(std::forward<F>(fun), std::forward<C>(obj)...));
        return (this->derived());
    }
    ...
    /**
		 ...
		 * Function signature : void(zephyr::error_code ec)
		 */
    template<class F, class... C>
    ...
    /**
		 ...
		 * if fun is nonmember function, the obj param must be none, otherwise the obj must be the
		 * the class object's pointer or refrence.
		 * Function signature : void(std::shared_ptr<zephyr::udp_session>& session_ptr, asio::error_code ec)
		 */
    template<class F, class... C>
...
public:
    /**
		 ...
		 */
    inline asio::ip::udp::socket& acceptor()
    {
...
protected:
    ...
    inline void _post_stop(const error_code& ec, std::shared_ptr<derived_t> self_ptr, state_t old_state)
    {
        ...
        asio::post(this->io_.strand(), [this, ec, this_ptr = std::move(self_ptr), old_state]() {
            ...
            // stop all the sessions, the session::stop must be no blocking,otherwise it may be cause loop lock.
            this->sessions_.foreach ([](std::shared_ptr<session_t>& session_ptr) { session_ptr->stop(); });
    ...
    inline void _post_recv(condition_wrap<MatchCondition> condition)
    {
        ...
        try
        {
            this->acceptor_.async_receive_from(this->buffer_.prepare(this->buffer_.pre_size()), this->remote_endpoint_,
                asio::bind_executor(this->io_.strand(),
                    make_allocator(this->rallocator_, [this, condition](const error_code& ec, std::size_t bytes_recvd) {
...
//#ifdef XD_KCP
    ...
    inline void _handle_recv(const error_code& ec, std::size_t bytes_recvd, condition_wrap<MatchCondition> condition)
    {
        ...
        if (!ec)
        {
            ...
            // we create a new session and put it into the session_mgr pool
                ...
                if constexpr (std::is_same_v<MatchCondition, use_kcp_t>)
                {
                    ...
                    if (kcp::is_kcphdr_syn(s))
                    {
                        ...
                        auto task = [this, ec, condition, session_ptr,
                                        syn = std::string{s.data(), s.size()}]() mutable {
...
//#endif // XD_KCP
    ...
    {
        return std::make_shared<session_t>(std::forward<Args>(args)..., this->sessions_, this->listener_, this->io_,
            this->buffer_.pre_size(), this->buffer_.max_size(), this->buffer_, this->acceptor_, this->remote_endpoint_, this->silence_timeout_);
    }

    template<typename MatchCondition>
    inline void _handle_accept(const error_code& ec, std::string_view first, std::shared_ptr<session_t> session_ptr,
...
protected:
    ...
    /// buffer
    zephyr::buffer_wrap<zephyr::linear_buffer> buffer_;

    std::size_t silence_timeout_ = udp_silence_timeout;
};
} // namespace zephyr::detail
...
#endif // !__ZEPHYR_UDP_SERVER_HPP__
