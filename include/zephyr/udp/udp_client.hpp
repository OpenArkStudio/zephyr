...
#ifndef __ZEPHYR_UDP_CLIENT_HPP__
#define __ZEPHYR_UDP_CLIENT_HPP__

#if defined(_MSC_VER) && (_MSC_VER >= 1200)
#pragma once
#endif // defined(_MSC_VER) && (_MSC_VER >= 1200)

#include <zephyr/base/client.hpp>
...
#include <zephyr/udp/component/kcp_stream_cp.hpp>
    ...
    , public udp_send_op<derived_t, false>
{
    ...
    template<class, bool>
    friend class reconnect_timer_cp;
    template<class, bool>
    friend class connect_timeout_cp;
    template<class, class>
    friend class connect_cp;
    template<class>
...
public:
    ...
    /**
		 * @constructor
		 */
    explicit udp_client_impl_t(std::size_t init_buffer_size = udp_frame_size,
    ...
    bool start(String&& host, StrOrInt&& port)
    {
        return this->derived().template _do_connect<false>(
            std::forward<String>(host), std::forward<StrOrInt>(port), condition_wrap<void>{});
    }
    ...
    {
        return this->derived().template _do_connect<false>(
            std::forward<String>(host), std::forward<StrOrInt>(port), condition_wrap<use_kcp_t>(c));
    }
...
protected:
    ...
    bool _do_connect(String&& host, StrOrInt&& port, condition_wrap<MatchCondition> condition)
    {
        ...
        try
        {
            ...
            this->derived()._make_reconnect_timer(std::shared_ptr<derived_t>{}, [this, h = to_string(host),
                                                                                    p = to_string(port), condition]() {
                state_t expected = state_t::stopped;
                if (this->state_.compare_exchange_strong(expected, state_t::starting))
                {
                    auto task = [this, h, p, condition]() {
                        this->derived().template _start_connect<true>(h, p, std::shared_ptr<derived_t>{}, condition);
                    };
            ...
            return this->derived().template _start_connect<isAsync>(
                std::forward<String>(host), std::forward<StrOrInt>(port), std::shared_ptr<derived_t>{}, condition);
        }
        ...
        try
        {
            this->socket_.async_receive(this->buffer_.prepare(this->buffer_.pre_size()),
                asio::bind_executor(this->io_.strand(),
                    make_allocator(this->rallocator_, [this, self_ptr = std::move(this_ptr), condition](
        ...
        condition_wrap<MatchCondition> condition)
    {
        ...
        if (!ec)
        {
            ...
            else
            {
                ...
                /*else if (kcp::is_kcphdr_syn(s, this->kcp_->seq_))
                {
                    ZEPHYR_ASSERT(false);
                }*/
...
namespace zephyr {
class udp_client : public detail::udp_client_impl_t<udp_client, asio::ip::udp::socket, zephyr::linear_buffer>
{
public:
    using udp_client_impl_t<udp_client, asio::ip::udp::socket, zephyr::linear_buffer>::udp_client_impl_t;
};
} // namespace zephyr

#endif // !__ZEPHYR_UDP_CLIENT_HPP__
