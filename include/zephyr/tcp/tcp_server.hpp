...
#ifndef __ZEPHYR_TCP_SERVER_HPP__
#define __ZEPHYR_TCP_SERVER_HPP__
...
namespace zephyr::detail
{
	...
	public:
		...
		inline void stop()
		{
			...
			ZEPHYR_ASSERT(this->state_ == state_t::stopped);
		}
	...
	public:
		 ...
		 * Function signature : void(std::shared_ptr<zephyr::tcp_session>& session_ptr, std::string_view s)
		 */
		 ...
		 * Function signature : void(std::shared_ptr<zephyr::tcp_session>& session_ptr)
		 */
		template<class F, class ...C>
	...
	public:
		/**
		 * @function : get the acceptor reference
		 */
	...
	protected:
		...
		inline void _post_stop(const error_code& ec, std::shared_ptr<derived_t> self_ptr, state_t old_state)
		{
			...
			asio::post(this->io_.strand(), [this, ec, this_ptr = std::move(self_ptr), old_state]()
			{
				...
				this->sessions_.foreach([](std::shared_ptr<session_t> & session_ptr)
				{
					session_ptr->stop();
				});
		...
		inline std::shared_ptr<session_t> _make_session(Args&&... args)
		{
			return std::make_shared<session_t>(std::forward<Args>(args)..., this->sessions_, this->listener_,
				this->iopool_.get(), this->init_buffer_size_, this->max_buffer_size_, this->silence_timeout_);
		}
	...
	protected:
        ...
        std::size_t             silence_timeout_ = tcp_silence_timeout;
	};
}
...
#endif // !__ZEPHYR_TCP_SERVER_HPP__
