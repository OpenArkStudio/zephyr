...
#ifndef __ZEPHYR_TCP_SEND_OP_HPP__
#define __ZEPHYR_TCP_SEND_OP_HPP__
...
namespace zephyr::detail
{
	...
	protected:
		...
		inline bool _tcp_send_dgram(BufferSequence&& buffer, Callback&& callback)
		{
			...
			{
				ZEPHYR_ASSERT(buffer.size() > (std::numeric_limits<std::uint16_t>::max)());
				bytes = 9;
				head = std::make_unique<std::uint8_t[]>(bytes);
...
#endif // !__ZEPHYR_TCP_SEND_OP_HPP__
