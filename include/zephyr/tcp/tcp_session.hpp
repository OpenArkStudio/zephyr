...
#ifndef __ZEPHYR_TCP_SESSION_HPP__
#define __ZEPHYR_TCP_SESSION_HPP__
...
namespace zephyr::detail
{
	...
	public:
		...
		explicit tcp_session_impl_t(
			...
			, wallocator_()
		{
			this->silence_timeout(std::chrono::milliseconds(silence_timeout));
		}

		/**
		 * @destructor
		 */
		~tcp_session_impl_t() = default;
	...
	protected:
		...
		{
			// to avoid the user call stop in another thread,then it may be socket_.async_read_some
...
#endif // !__ZEPHYR_TCP_SESSION_HPP__
