...
#ifndef __ZEPHYR_TCP_CLIENT_HPP__
#define __ZEPHYR_TCP_CLIENT_HPP__
...
#include <zephyr/tcp/component/tcp_keepalive_cp.hpp>
#include <zephyr/tcp/impl/tcp_send_op.hpp>
#include <zephyr/tcp/impl/tcp_recv_op.hpp>
    ...
    , public tcp_recv_op<derived_t, false>
{
    ...
    template<class, bool>
    friend class tcp_send_op;
    template<class, bool>
    friend class tcp_recv_op;
...
public:
    /**
		 * @constructor
		 */
    explicit tcp_client_impl_t(std::size_t init_buffer_size = tcp_frame_size,
    ...
    {
        return this->derived().template _do_connect<false>(std::forward<String>(host), std::forward<StrOrInt>(port),
    ...
    bool start(String&& host, StrOrInt&& port, MatchCondition condition)
    {
        return this->derived().template _do_connect<false>(
            std::forward<String>(host), std::forward<StrOrInt>(port), condition_wrap<MatchCondition>(condition));
    }
    ...
    {
        return this->derived().template _do_connect<true>(std::forward<String>(host), std::forward<StrOrInt>(port),
            condition_wrap<asio::detail::transfer_at_least_t>{asio::transfer_at_least(1)});
    }
...
protected:
    ...
    template<typename MatchCondition>
    inline void _handle_recv(const error_code& ec, std::size_t bytes_recvd, std::shared_ptr<derived_t> this_ptr,
...
#endif // !__ZEPHYR_TCP_CLIENT_HPP__
