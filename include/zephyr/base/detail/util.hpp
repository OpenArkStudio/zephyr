...
#ifndef __ZEPHYR_UTIL_HPP__
#define __ZEPHYR_UTIL_HPP__
...
static long constexpr tcp_silence_timeout = 15 * 1000;
static long constexpr udp_silence_timeout = 15 * 1000;
static long constexpr http_silence_timeout = 15 * 1000;
...
/*
	 ...
	 * the frame header.
	 * referenced from beast stream.hpp
	 */
static std::size_t constexpr tcp_frame_size = 1536;
static std::size_t constexpr udp_frame_size = 1536;
...
struct ignore
{
    ...
    template<class... Args>
    static inline constexpr void unused(Args&&...) noexcept
    {
    }
};
...
public:
    ...
    copyable_wrapper(Args&&... args)
        : raw(std::forward<Args>(args)...)
    {
    }
    template<typename = void>
    copyable_wrapper(T&& o)
        : raw(std::move(o))
    {
    }
    ...
    copyable_wrapper(copyable_wrapper const& r)
        : raw(const_cast<T&&>(r.raw))
    {
        throw 0;
    }
    copyable_wrapper& operator=(copyable_wrapper const& r)
    {
        raw = const_cast<T&&>(r.raw);
        throw 0;
    }
...
template<typename T>
struct is_copyable_wrapper<T,
    std::void_t<typename T::value_type,
        typename std::enable_if_t<std::is_same_v<T, copyable_wrapper<typename T::value_type>>>>> : std::true_type
{
};
...
{
public:
    id_maker(T init = static_cast<T>(1))
        : id(init)
    {
...
template<typename T>
struct is_string<T,
    std::void_t<typename T::value_type, typename T::traits_type, typename T::allocator_type,
...
struct is_string_view<T, std::void_t<typename T::value_type, typename T::traits_type,
                             typename std::enable_if_t<std::is_same_v<T,
                                 std::basic_string_view<typename T::value_type, typename T::traits_type>>>>>
    : std::true_type
{
};
...
inline Protocol to_endpoint(String&& host, StrOrInt&& port)
{
    ...
    if /**/ constexpr (std::is_same_v<asio::ip::udp::endpoint, Protocol>)
    {
        ...
        asio::ip::udp::endpoint endpoint = *resolver.resolve(h, p, asio::ip::resolver_base::flags::address_configured);
        return endpoint;
    }
    else if constexpr (std::is_same_v<asio::ip::tcp::endpoint, Protocol>)
    {
        asio::ip::tcp::resolver resolver(ioc);
        asio::ip::tcp::endpoint endpoint = *resolver.resolve(h, p, asio::ip::resolver_base::flags::address_configured);
        return endpoint;
    }
    else
    {
...
struct hash<asio::ip::udp::endpoint>
{
    ...
    inline result_type operator()(argument_type const& s) const noexcept
    {
        ...
        return zephyr::detail::bkdr_hash((const unsigned char*)(&s), sizeof(argument_type));
    }
};
