...
#ifndef __ZEPHYR_ERROR_HPP__
#define __ZEPHYR_ERROR_HPP__
...
namespace zephyr
{
	...
	#ifdef ZEPHYR_ASSERT
		static_assert(false, "Unknown ZEPHYR_ASSERT definition will affect the relevant functions of this program.");
	#else
		#if defined(_DEBUG) || defined(DEBUG)
			#define ZEPHYR_ASSERT(x) assert(x);
		#else
	 ...
	 * 
	 * the zephyr custom error code rule : the highest 8 bits is 0,the 23 bit is 1,
	 *
	 * the http_parser error code rule : the highest 9 bits is 0,the 22 bit is 1,
	 *
	 * we check the error_category like this : 
	 * if the 23 bit is 1,it is zephyr_category.
