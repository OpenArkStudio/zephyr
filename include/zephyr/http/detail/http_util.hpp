...
#ifndef __ZEPHYR_HTTP_UTIL_HPP__
#define __ZEPHYR_HTTP_UTIL_HPP__
...
#include <zephyr/http/detail/http_parser.h>
#include <zephyr/http/detail/mime_types.hpp>
	...
	namespace
	{
		...
		request<Body, Fields> make_request(std::string_view uri, error_code& ec)
		{
			...
			try
			{
				...
				{
					ZEPHYR_ASSERT(!has_unencode_char(uri));
		...
		{
			ZEPHYR_ASSERT(!has_unencode_char(target));
			request<Body, Fields> req;
			// Set up an HTTP GET request message
			req.method(method);
			req.version(version);
...
#endif // !__ZEPHYR_HTTP_UTIL_HPP__
