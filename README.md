...
> 本库是从原 [asio2](https://github.com/zhllxt/asio2) 的基础上修改和增加了部分代码和例子，用在ARK框架中，请各位遵循原库开源协议。  
> 为了减小仓库体积，重新修改了文件结构，去掉了无用文件，通过`submodule`和`cmake find_package`的方式来获取，所以重新上传了一次。当前同步到源仓库的`git commit id = ce6ac7f6ba2a931a19b9abb7627d12cfaeed8ab1`, 后续会持续从源仓库合并。
...
zephyr框架已经处理过了这些问题,你可以在如MFC的OnInitDialog等地方调用server.start(...),
...
字符串大小为128时:rest_rpc的qps约为30000/秒,zephyr的qps约为37000/秒,此时zephyr比rest_rpc性能约高19%.
字符串越来越大时(测试了16K和64K),两者性能基本相同.
```
rpc测试的和说明代码请看:[rpc性能测试代码](https://github.com/zhllxt/zephyr/wiki/rpc%E6%80%A7%E8%83%BD%E6%B5%8B%E8%AF%95%E4%BB%A3%E7%A0%81)
...
//server.start("0.0.0.0", "8080", zephyr::use_dgram); // 数据报模式的TCP,无论发送多长的数据,双方接收的一定是相应长度的整包数据
```
##### 客户端:
```c++
...
//client.start("0.0.0.0", "8080", zephyr::use_dgram); // 数据报模式的TCP,无论发送多长的数据,双方接收的一定是相应长度的整包数据
...
//server.start("0.0.0.0", "8080", zephyr::use_kcp); // 可靠UDP
```
##### 客户端:
```c++
...
//client.async_start("0.0.0.0", "8080", zephyr::use_kcp); // 可靠UDP
```
...
int add(std::shared_ptr<zephyr::rpc_session>& session_ptr, int a, int b)
{
	return a + b;
}
...
## SSL:
##### TCP/HTTP/WEBSOCKET均支持SSL功能(需要在config.hpp中将#define ZEPHYR_USE_SSL宏定义放开)
```c++
zephyr::tcps_server server;
// 设置证书模式
